...
export default function StarLoader() {
  useEffect(() => {
    ...
    const g1 = ctx.createLinearGradient(0, 0, size, size); g1.addColorStop(0.1886, '#348CFF'); g1.addColorStop(0.8114, '#1EECB2');
    const g2 = ctx.createLinearGradient(0, size, size, 0); g2.addColorStop(0, '#2F63C5'); g2.addColorStop(0.5428, '#2882FA');
    const g3 = ctx.createLinearGradient(0, 0, size, 0); g3.addColorStop(0, '#2F63C5'); g3.addColorStop(0.8168, '#2882FA');
    const g4 = ctx.createLinearGradient(0, 0, size, size); g4.addColorStop(0.1886, '#28A2B1'); g4.addColorStop(0.8114, '#1EECB2');
    ...
    function drawStarFromPath(cx, cy, scale, color, rotation = 0, opacity = 1) {
      ctx.save();
    ...
    function init() {
      ...
      starLayout.forEach(config => {
        ...
        if (config.type === 'major') majorStars.push(star);
        else satelliteStars.push(star);
      });
    }
    ...
    function animate() {
      ...
      const allStars = [...majorStars, ...satelliteStars];
      allStars.sort((a, b) => (a.y > b.y ? 1 : -1));
      allStars.forEach(star => star.update(frame, { x: orbitCenterX, y: orbitCenterY }));

      requestAnimationFrame(animate);
    }

    const typingText = "GERANDO PERFIL COM IA...";
    let charIndex = 0;
